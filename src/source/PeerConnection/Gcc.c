    ...
    switch (pRateCtrl->state) {
        ...
        case GCC_STATE_DECREASE:
            ...
            if (pRateCtrl->avgMaxBitrate == 0.0) {
                ...
                pRateCtrl->avgMaxBitrate = GCC_CONVERGENCE_SMOOTHING * pRateCtrl->avgMaxBitrate +
                                           (1.0 - GCC_CONVERGENCE_SMOOTHING) * pRateCtrl->A_hat;
                pRateCtrl->varMaxBitrate = GCC_CONVERGENCE_SMOOTHING * pRateCtrl->varMaxBitrate +
                                           (1.0 - GCC_CONVERGENCE_SMOOTHING) * diff * diff;
            }
            break;
...
STATUS gccOnTwccPacketReports(PGccController pController, PTwccPacketReport pReports, UINT32 reportCount, UINT64 rttEstimateKvs)
{
    ...
    while (gccFinalizeGroup(pController, &d_i)) {
        ...
        CHK_STATUS(gccUpdateDelayController(&pController->rateCtrl, signal, dtKvs, incomingBitrate, pController->minBitrate,
                                            pController->maxBitrate));
    }
