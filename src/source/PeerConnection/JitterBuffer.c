    ...
    for (; index != lastIndex; index++) {
        ...
        if (!hasEntry) {
            ...
            if (curTimestamp != pJitterBuffer->headTimestamp) {
                ...
                else if (seenHeadFramePacket && startDropIndex != index && (pJitterBuffer->headTimestamp < earliestAllowedTimestamp || bufferClosed)) {
            ...
            if (curTimestamp == pJitterBuffer->headTimestamp && pCurPacket->header.marker && containStartForEarliestFrame &&
                headFrameIsContiguous) {
