...
STATUS freePeerConnection(PRtcPeerConnection* ppPeerConnection)
{
    ...
    if (IS_VALID_TIMER_QUEUE_HANDLE(pKvsPeerConnection->timerQueueHandle)) {
        if (pKvsPeerConnection->twccFeedbackTimerId != MAX_UINT32) {
            timerQueueCancelTimer(pKvsPeerConnection->timerQueueHandle, pKvsPeerConnection->twccFeedbackTimerId, (UINT64) pKvsPeerConnection);
...
STATUS closePeerConnection(PRtcPeerConnection pPeerConnection)
{
    ...
    if (IS_VALID_TIMER_QUEUE_HANDLE(pKvsPeerConnection->timerQueueHandle) && pKvsPeerConnection->twccFeedbackTimerId != MAX_UINT32) {
        timerQueueCancelTimer(pKvsPeerConnection->timerQueueHandle, pKvsPeerConnection->twccFeedbackTimerId, (UINT64) pKvsPeerConnection);
        pKvsPeerConnection->twccFeedbackTimerId = MAX_UINT32;
    }
