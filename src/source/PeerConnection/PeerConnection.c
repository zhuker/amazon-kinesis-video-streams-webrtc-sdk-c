...
STATUS onFrameDroppedFunc(UINT64 customData, UINT16 startIndex, UINT16 endIndex, UINT32 timestamp)
{
    ...
    for (index = startIndex; UINT16_DEC(index) != endIndex; index++) {
        ...
        if (hasEntry) {
            ...
            if (STATUS_FAILED(pTransceiver->pJitterBuffer->depayPayloadFn(pPacket->payload, pPacket->payloadLength, NULL, &partialFrameSize, &depayIsFirst))) {
    ...
    for (index = startIndex; UINT16_DEC(index) != endIndex; index++) {
        ...
        if (hasEntry) {
            ...
            BOOL depayIsFirst = isFirstInFrame;
            if (STATUS_FAILED(pTransceiver->pJitterBuffer->depayPayloadFn(pPacket->payload, pPacket->payloadLength, pCurPtrInFrame, &partialFrameSize, &depayIsFirst))) {
...
VOID onSctpSessionDataChannelMessage(UINT64 customData, UINT32 channelId, BOOL isBinary, PBYTE pMessage, UINT32 pMessageLen)
{
    ...
    __atomic_fetch_add(&pKvsDataChannel->atomicBytesReceived, (UINT64) pMessageLen, __ATOMIC_RELAXED);
