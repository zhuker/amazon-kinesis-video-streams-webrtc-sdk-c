...
STATUS createTwccReceiverManager(PTwccReceiverManager* ppManager)
{
    ...
    CHK_STATUS(hashTableCreateWithParams(TWCC_RECEIVER_HASH_BUCKET_COUNT, TWCC_RECEIVER_HASH_BUCKET_LENGTH,
                                         &pManager->pReceivedPktsHashTable));
...
STATUS twccReceiverOnPacketReceived(PKvsPeerConnection pKvsPeerConnection, PRtpPacket pRtpPacket)
{
    ...
    pTwccExtData = findOneByteExtension(pRtpPacket->header.extensionPayload, pRtpPacket->header.extensionLength,
                                        (UINT8) pKvsPeerConnection->twccExtId, NULL);
...
STATUS sendRtcpTwccFeedback(PKvsPeerConnection pKvsPeerConnection)
{
    ...
    UINT64 lastArrivalTimeKvs = (UINT64)(currentTime / TWCC_REFERENCE_TIME_DIVISOR) * TWCC_REFERENCE_TIME_DIVISOR * HUNDREDS_OF_NANOS_IN_A_MILLISECOND;
    DLOGD("TWCC ref: refTimeKvs=%llu currentTime=%u refTime24=%u lastArrival=%llu",
          (unsigned long long)referenceTimeKvs, currentTime, referenceTime24, (unsigned long long)lastArrivalTimeKvs);
    ...
    for (i = 0; i < packetStatusCount; i++) {
        if (STATUS_SUCCEEDED(hashTableGet(pManager->pReceivedPktsHashTable, seqNum, &packetInfoValue))) {
            ...
            if (status == TWCC_STATUS_SYMBOL_NOTRECEIVED) {
                DLOGD("TWCC delta overflow: seq=%u i=%u delta=%d arrival=%llu la    stArrival=%llu",
                      seqNum, i, deltaTicks, (unsigned long long)pPacketInfo->arrivalTimeKvs,
                      (unsigned long long)lastArrivalTimeKvs);
                lostCount++;
            } else {
                receivedCount++;
                if (deltaTicks < minDelta) minDelta = deltaTicks;
                if (deltaTicks > maxDelta) maxDelta = deltaTicks;
            }
    ...
    DLOGD("TWCC feedback: base=%u count=%u received=%u lost=%u minDelta=%d maxDelta=%d",
          baseSeqNum, packetStatusCount, receivedCount, lostCount, minDelta, maxDelta);
    ...
    while (i < packetStatusCount) {
        ...
        runLength = 1;
        while (i + runLength < packetStatusCount &&
               pStatuses[i + runLength] == runStatus &&
               runLength < TWCC_MAX_PACKET_STATUS_COUNT) {
            runLength++;
        }
