...
static VOID sctpFlushSendQueue(PSctpAssociation pAssoc, SctpAssocOutboundPacketFn outboundFn, UINT64 outboundCustomData)
{
    ...
    for (i = 0; i < SCTP_MAX_QUEUED_MESSAGES; i++) {
        ...
        sctpAssocSend(pAssoc, pAssoc->sendQueue[i].streamId, pAssoc->sendQueue[i].ppid, pAssoc->sendQueue[i].unordered,
                      pAssoc->sendQueue[i].payload, pAssoc->sendQueue[i].payloadLen, pAssoc->sendQueue[i].maxRetransmits,
                      pAssoc->sendQueue[i].lifetimeMs, outboundFn, outboundCustomData);
...
static STATUS sctpHandleInit(PSctpAssociation pAssoc, PBYTE pValue, UINT32 valueLen, SctpAssocOutboundPacketFn outboundFn,
                              UINT64 outboundCustomData)
{
    ...
    offset += sctpWriteInitAckChunk(pAssoc->outPacket + offset, pAssoc->myVerificationTag, SCTP_DEFAULT_ARWND, SCTP_MAX_STREAMS,
                                    SCTP_MAX_STREAMS, pAssoc->nextTsn, (PBYTE) &cookie, SCTP_COOKIE_SIZE);
...
static STATUS sctpHandleSack(PSctpAssociation pAssoc, PBYTE pValue, UINT32 valueLen, SctpAssocOutboundPacketFn outboundFn,
                              UINT64 outboundCustomData)
{
...
static STATUS sctpChunkDispatch(UINT8 type, UINT8 flags, PBYTE pValue, UINT32 valueLen, UINT64 customData)
{
    ...
    switch (type) {
        ...
        case SCTP_CHUNK_HEARTBEAT:
            ...
            {
                UINT32 offset =
                    sctpWriteCommonHeader(pCtx->pAssoc->outPacket, pCtx->pAssoc->localPort, pCtx->pAssoc->remotePort, pCtx->pAssoc->peerVerificationTag);
...
STATUS sctpAssocHandlePacket(PSctpAssociation pAssoc, PBYTE pBuf, UINT32 bufLen, SctpAssocOutboundPacketFn outboundFn,
                             UINT64 outboundCustomData, SctpAssocMessageFn messageFn, UINT64 messageCustomData)
{
    STATUS retStatus = STATUS_SUCCESS;
...
STATUS sctpAssocSend(PSctpAssociation pAssoc, UINT16 streamId, UINT32 ppid, BOOL unordered, PBYTE pPayload, UINT32 payloadLen,
                     UINT16 maxRetransmits, UINT64 lifetimeMs, SctpAssocOutboundPacketFn outboundFn, UINT64 outboundCustomData)
{
    UINT32 offset;
    UINT32 tsn;
    UINT16 ssn = 0;
